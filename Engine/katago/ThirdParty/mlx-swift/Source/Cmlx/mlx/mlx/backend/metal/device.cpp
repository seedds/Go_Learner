...
Device::Device() {
  ...
  if (arch_.empty()) {
    // The iOS/visionOS simulator's Metal returns a null architecture name;
    // std::string(nullptr) would crash. Guard it and fall back to a generic
    // Apple-GPU arch string so the downstream parsing stays valid. Harmless on
    // real devices, where utf8String() is non-null.
    auto* arch_name = device_->architecture()->name();
    const char* arch_cstr = arch_name ? arch_name->utf8String() : nullptr;
    arch_ = arch_cstr ? std::string(arch_cstr) : std::string("applegpu_g14g");
  }
  int ag_tens = 0;
  int ag_ones = 0;
  if (arch_.size() >= 3) {
